...
try {
  ...
  if (fs.existsSync(i18nConfigPath)) {
    ...
    console.log(`   - Détection automatique: ${i18nConfig.i18n.localeDetection ? 'Activée' : 'Désactivée'}`);

    if (i18nConfig.i18n.localeDetection) {
      console.log(`${colors.yellow}⚠️ La détection automatique de la langue est activée, ce qui peut causer des redirections infinies.${colors.reset}`);
      console.log(`   Considérez désactiver cette option en mettant 'localeDetection: false' dans next-i18next.config.js.`);
    } else {
      console.log(`${colors.green}✅ La détection automatique de la langue est désactivée, ce qui est recommandé.${colors.reset}`);
    }
  } else {
  ...
  console.error(`${colors.red}❌ Erreur lors de la vérification de la configuration i18n: ${error.message}${colors.reset}\n`);
}
...
try {
  ...
  if (fs.existsSync(nextConfigPath)) {
    ...
    if (nextConfigContent.includes('redirects')) {
      console.log(`${colors.yellow}⚠️ Des redirections sont configurées dans next.config.js, ce qui pourrait causer des problèmes.${colors.reset}`);
    } else {
      console.log(`${colors.green}✅ Aucune redirection trouvée dans next.config.js.${colors.reset}`);
    }
  } else {
  ...
  console.error(`${colors.red}❌ Erreur lors de la vérification des redirections: ${error.message}${colors.reset}\n`);
}

// Conseils pour résoudre les problèmes de redirection
console.log(`${colors.bright}${colors.blue}🔧 Conseils pour résoudre les problèmes de redirection:${colors.reset}`);
...
console.log(`${colors.bright}${colors.blue}🚀 Exécutez 'npm run dev:clean' pour redémarrer le serveur de développement.${colors.reset}`);
